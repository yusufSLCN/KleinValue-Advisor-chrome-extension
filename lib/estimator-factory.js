...
async function getEstimator() {
    ...
    const providerId = settings.provider || 'puter';
    ...
    if (requiresApiKey && !apiKey) {
        throw new Error(
            `API key not configured for ${providerConfig.name}. Please set it in extension settings.`
        );
    }
    ...
    if (shouldCreate) {
        console.log(
            'Creating new estimator for provider:',
            providerId,
            'model:',
            settings.modelName
        );
        ...
        console.log(
            'Reusing existing estimator for provider:',
            providerId,
            'model:',
            settings.modelName
        );
