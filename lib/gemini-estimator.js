...
class GeminiEstimator {
    ...
    async estimateValue(itemData) {
        ...
        const inlineCount = withImages ? images.length : 0;
        console.log(
            `Including ${inlineCount} inline image${inlineCount === 1 ? '' : 's'} in request (max ${this.maxImages}).`
        );
        try {
            const res = await fetch(
                `https://generativelanguage.googleapis.com/v1beta/models/${this.modelName}:generateContent?key=${encodeURIComponent(this.apiKey)}`,
                {
                    method: 'POST',
            ...
            if (!res.ok) {
                ...
                if (res.status === 429) {
                    const quotaDetails = this.parseQuotaErrorDetails(errText);
                    const userMessage =
                        quotaDetails.userMessage ||
                        'Gemini API quota limit reached. Please wait before trying again.';
                    const quotaError = new Error(userMessage);
                    quotaError.code = 'gemini-quota-exceeded';
                    quotaError.retryAfterSeconds = quotaDetails.retryAfterSeconds;
                    quotaError.httpStatus = 429;
                    quotaError.rawResponse = errText;
                    quotaError.userMessage = userMessage;
                    throw quotaError;
                }
    ...
    parseQuotaErrorDetails(errText) {
        if (!errText || typeof errText !== 'string') {
            return {};
        }
        try {
            const parsed = JSON.parse(errText);
            const userMessage = parsed?.error?.message;
            ...
            console.warn('Failed to parse quota error details:', err);
            return {};
        }
    }

    parseRetryDelay(delayValue) {
        if (!delayValue) {
            return undefined;
        }
        if (typeof delayValue === 'string') {
            const match = delayValue.match(/([\d.]+)\s*s?/i);
            if (match) {
        ...
        if (typeof delayValue === 'object') {
            const seconds = Number(delayValue.seconds ?? 0);
            const nanos = Number(delayValue.nanos ?? 0);
            const total = (Number.isFinite(seconds) ? seconds : 0) + (Number.isFinite(nanos) ? nanos / 1e9 : 0);
