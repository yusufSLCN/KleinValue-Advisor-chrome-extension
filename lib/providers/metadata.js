...
function prettifyModelId(id) {
    return id
        .replace(/-/g, ' ')
        .replace(/\b(gpt|claude|sonnet|opus|haiku|mini|flash|pro)\b/gi, match => match[0].toUpperCase() + match.slice(1))
        .replace(/\s+/g, ' ')
        .trim()
        .replace(/^Gpt/i, 'GPT')
...
function isImageOnlyGeminiModel(model) {
    ...
    return (model.supportedGenerationMethods || []).some(method => method.toLowerCase().includes('image'));
}
