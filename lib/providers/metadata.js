const PROVIDERS = [
    ...
    },
    {
        ...
        icon: '🧩',
        tagline: 'Serverless multi-vendor AI via Puter.js.',
        docsUrl: 'https://docs.puter.com/',
        defaultModel: 'gpt-5-nano',
        placeholderKey: 'No API key required',
        supportsVision: true,
...
async function fetchPuterModels() {
    const puter = getPuterClientForMetadata();
    ...
    return list
        ...
        .map((model) => ({
            ...
            description: model.provider
                ? `Provider: ${String(model.provider).toUpperCase()}`
                : 'Puter model'
        }))
        .sort((a, b) => a.label.localeCompare(b.label));
}

function getPuterClientForMetadata() {
